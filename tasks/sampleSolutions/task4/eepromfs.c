...
void open_for_write(filename fn) {
    ...
    if (start != FREE_BLOCK) { /* File exits */
...
void close(filename fn) {
    if (fsmeta.dir[fn].startblock == FREE_BLOCK) {
	printf( "ERROR close: File %d does not exist.\n", fn);
        return;
    }
    if (fsmeta.dir[fn].currpos == CLOSED_FILE) {
	printf( "ERROR close: File %d not open.\n", fn);
	return;
    }
...
void write(filename fn, uint8_t *buff, uint16_t len) {
    ...
    if (fsmeta.dir[fn].currpos == CLOSED_FILE) {
	    printf( "ERROR write: File %d not open.\n", fn);
	    return;
    }

    for (loc=fsmeta.dir[fn].currpos, idx=0;
           idx < len; idx++, loc++) {
